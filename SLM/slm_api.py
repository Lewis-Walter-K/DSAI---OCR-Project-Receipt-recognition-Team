...
def process_ocr_with_gemini(ocr_text: str) -> InvoiceStructuredOutput:
    ...
    if not MY_API_KEY:
        ...
        return InvoiceStructuredOutput(date="UNKNOWN", total_amount=0.0, currency="VND")
    ...
    system_instruction = (
        ...
        "You must respond ONLY with a valid JSON object. You are FORBIDDEN to use any keys other than the following 3 keys:\n"
        '1. "date" (string): The date in YYYY-MM-DD format. Note: Most receipts are from Asia/Europe where dates are in DD/MM/YYYY format, NOT US format! So 08/06/2026 means June 8th (2026-06-08).\n'
        '2. "total_amount" (float): The final paid amount. MUST be a pure number without commas or currency symbols (e.g., 47000 or 47000.5).\n'
        '3. "currency" (string): The ISO currency code.\n'
